...
export const accessoriesFilters = {
    types: ['Sunglasses', 'Belt', 'Purse', 'Clutch'],
    brands: ['Gucci', 'Prada', 'Ray-Ban', 'Coach', 'Michael Kors', 'Kate Spade'],
    materials: ['Leather', 'Metal', 'Canvas', 'Fabric', 'Synthetic'],
    colors: ['Black', 'Brown', 'Gold', 'Silver', 'Blue', 'Red'],
    priceRanges: [
        { min: 0, max: 1000 },
        { min: 1000, max: 3000 },
        { min: 3000, max: 5000 },
...
const Accessories = () => {
    ...
    useEffect(() => {
        const fetchAccessories = async () => {
            try {
                ...
                const accessoryItems = querySnapshot.docs.map((doc) => ({
                    id: doc.id,
        ...
        return items.filter(item => {
            ...
            if (selectedCategory !== 'all') {
                switch (selectedCategory) {
                    case 'sunglasses':
                        if (item.AccessoryType !== 'Sunglasses') return false;
                        break;
                    case 'belt':
                        if (item.AccessoryType !== 'Belt') return false;
                        break;
                    case 'purse':
                        if (item.AccessoryType !== 'Purse') return false;
                        break;
                    case 'clutch':
                        if (item.AccessoryType !== 'Clutch') return false;
                        break;
                }
            }
    ...
    const filteredFashion = sortFashion(filterAccessories(allFashion));
                ...
                <div className="flex justify-center gap-6 mt-4 flex-wrap">
                    ...
                    {[
                        ...
                        <button
                            ...
                            className={`text-sm uppercase hover:text-gray-900 transition-colors ${selectedCategory === category.id ? 'font-medium border-b-2 border-gray-700' : 'text-gray-600'
                                }`}
                        >
                            {category.label}
                        </button>
    ...
    const FiltersSidebarComponent = ({ isMobile = false }) => (
        <FiltersSidebar
            category="accessories"
                ...
                <div className="lg:hidden fixed inset-0 z-50 bg-white">
                    <div className="h-full flex flex-col">
                        <div className="flex justify-between items-center p-4 border-b border-gray-200">
                            <h2 className="text-xl font-semibold text-gray-800">Accessories Filters</h2>
                            <button
                        ...
                        <div className="flex-1 overflow-y-auto p-4">
                            <FiltersSidebarComponent isMobile={true} />
                        </div>
                    </div>
                </div>
            )}
